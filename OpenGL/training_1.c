...
void show_pict(void)//画面に描く内容
{
	...
	for(y = 0; y < height; y++){
		...
		for(x = 0; x < width; x++){
			...
			g = colorimage[y][x][1];
			r = colorimage[y][x][2];
			glcolor(0, 0, b);	//(r, g, b)の色で
			gpnt(x - width, y);		//(x, y)座標に点を打つ
		}
		for(x = 0; x < width; x++){
			b = colorimage[y][x][0];
			g = colorimage[y][x][1];
			r = colorimage[y][x][2];
			glcolor(0, g, 0);	//(r, g, b)の色で
			gpnt(x , y - height);		//(x, y)座標に点を打つ
		}
		for(x = 0; x < width; x++){
			b = colorimage[y][x][0];
			g = colorimage[y][x][1];
			r = colorimage[y][x][2];
			glcolor(r, 0, 0);	//(r, g, b)の色で
			gpnt(x - width, y - height);		//(x, y)座標に点を打つ
		}
	}
	glflush();
}
...
{
	int	 y, x, c;
	loadbmpfile ("Lenna.bmp", &cm, &height, &width, dat);
	printf("cm = %d \n", cm);
	printf("hite = %d \n", height);
	printf("wide = %d \n", width);
	for( c = 0; c < cm; c++){
